    ...
    new Promise(function(resolve) {
      socket.on('disconnect', resolve)
        ...
        .on('device.note', function(data) {
          ...
          //gejinjun
        .on('device.script', function(data) {
          return dbapi.setDeviceScript(data.serial, data.script)
            .then(function() {
              return dbapi.loadDevice(data.serial)
            })
            .then(function(device) {
        ...
        .on('device.APK_status', function(data) {
          return dbapi.setDeviceScriptStatus(data.serial, data.APK_status)
            .then(function() {
              return dbapi.loadDevice(data.serial)
            })
            .then(function(device) {
