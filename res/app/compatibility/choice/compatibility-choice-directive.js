var patchArray = require('../../device-list/util/patch-array')
...
, StandaloneService
  ...
  function DeviceItem() {
	...
	var check = a.lastChild
        ...
        if (device.usable&&device.APK_status!='monkey_start'&&device.APK_status!='install_start'&&device.APK_status!='runAir_start') {
         // a.href = '#!/control/' + device.serial
          li.classList.remove('device-is-busy')
	  check.setAttribute('serial', device.serial)
	  check.setAttribute('ip', device.provider.name)
	  check.className = 'monkey'
        }
        else {
  ...
  return {
    restrict: 'E'
  , template: require('./compatibility-choice.pug')
  , scope: {
      tracker: '&tracker'
    , columns: '&columns'
    , sort: '=sort'
...
/*	  if (device.APK_status=='monkey_start'||device.APK_status=='install_start'||device.APK_status=='runAir_start') {
	    kickDevice(device)
	    e.preventDefault()
	  }*/
        }
